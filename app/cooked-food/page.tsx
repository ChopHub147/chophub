...
export default function CookedFoodPage() {
  ...
  const sendOrderToWhatsApp = async () => {
    ...
    if (!whatsappWindow) {
      alert("Please allow pop-ups for ChopHub to continue to WhatsApp.");
      return;
    }

    let message = `*New Order - ChopHub Calabar*\n\n`;
    message += `*Customer Details*\n`;
    message += `Name: ${customerName}\n`;
    message += `Phone: ${customerPhone}\n`;
    message += `Delivery area: ${deliveryArea}\n`;
    ...
    message += `\n*Food subtotal: ₦${totalPrice.toLocaleString()}*\n`;
    message += `Delivery: ${
    ...
    try {
      ...
      whatsappWindow.close();
      alert("We could not save your order. Please check your connection and try again.");
      return;
    }
            ...
            {featuredDishes.map((dish) => (
              <div key={dish.id} className="contents">
                ...
                <div className="h-40 overflow-hidden md:h-52">
                  ...
                  {vendorNames[`meal-${dish.id}`]?.length > 0 && <p className="mb-3 text-xs font-semibold text-green-800">Sold by: {vendorNames[`meal-${dish.id}`].join(", ")}</p>}
